...
interface DashboardMetrics {
  content_pipeline: MetricValue
  engagement: MetricValue
  roi: MetricValue
  autonomy: MetricValue
}

// Fallback demo metrics shown when the database is empty or API fails
const FALLBACK_METRICS: DashboardMetrics = {
  ...
  autonomy: {
    value: 78,
    trend: { value: 'No escalations', positive: true },
  },
}

/** Return `true` if every top-level KPI value is 0 (empty database). */
function isMetricsEmpty(m: DashboardMetrics | null | undefined): boolean {
  if (!m) return true
  return (
    (m.content_pipeline?.value ?? 0) === 0 &&
    (m.engagement?.value ?? 0) === 0 &&
    (m.roi?.value ?? 0) === 0 &&
    (m.autonomy?.value ?? 0) === 0
  )
}
...
export default function HomePage() {
  ...
  // Use live data when available, otherwise fall back to demo numbers
  const kpi = metrics && !isMetricsEmpty(metrics) ? metrics : FALLBACK_METRICS
    ...
    >
      ...
      <div className='grid grid-cols-2 gap-4 lg:grid-cols-4'>
        <StatCard
          ...
          value={kpi.content_pipeline.value}
          icon={Icons.fileText}
          trend={kpi.content_pipeline.trend}
          colorClass='bg-brand-navy'
          delay={0.1}
        />
        <StatCard
          title='Success Rate'
          value={kpi.engagement.value}
          suffix='%'
          icon={Icons.activity}
          trend={kpi.engagement.trend}
        ...
        <StatCard
          title='AI Autonomy'
          value={kpi.autonomy.value}
          suffix='%'
          icon={Icons.sparkles}
          trend={kpi.autonomy.trend}
