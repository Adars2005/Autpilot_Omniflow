...
export function PolicyEditModal({ policy, isOpen, onClose, onSave }: PolicyEditModalProps) {
  ...
  const handleTranslate = useCallback(async () => {
    ...
    try {
      const result = await apiClient.post<{ dsl: PolicyDSL; confidence: number }>('/api/policies/translate', {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (policy) {
        savedPolicy = await apiClient.put<Policy>(`/api/policies/${policy.id}`, payload)
      } else {
