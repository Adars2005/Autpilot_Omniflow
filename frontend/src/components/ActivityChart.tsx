...
export function ActivityChart({
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const result = await apiClient.get<ActivityData[]>('/api/dashboard/activity')
        setData(result)
      } catch (e) {
        console.error(e)
      } finally {
  ...
  const totalSessions = data.length > 0 ? data.reduce((acc, d) => acc + d.sessions, 0) : 0
  const avgSuccess = data.length > 0 ? Math.round(
    data.reduce((acc, d) => acc + d.success, 0) / data.length
  ) : 0
  const trend = data.length > 0 && data[0].sessions > 0 ? (
    ((data[data.length - 1].sessions - data[0].sessions) / data[0].sessions) *
    100
  ).toFixed(1) : "0"
  ...
  return (
    <Card className={cn('relative overflow-hidden', className)}>
      ...
      <CardContent className='pt-0'>
        <div className='mt-4 h-[240px] w-full min-h-[240px] flex items-center justify-center'>
          {isLoading ? (
